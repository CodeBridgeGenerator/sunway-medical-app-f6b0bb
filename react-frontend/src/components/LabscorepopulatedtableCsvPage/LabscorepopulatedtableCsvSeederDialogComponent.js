...
import seederData from "./labscorepopulatedtableCsv.seeder.json";
...
const LabscorepopulatedtableCsvSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("labscorepopulatedtableCsv").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(LabscorepopulatedtableCsvSeederDialogComponent);
