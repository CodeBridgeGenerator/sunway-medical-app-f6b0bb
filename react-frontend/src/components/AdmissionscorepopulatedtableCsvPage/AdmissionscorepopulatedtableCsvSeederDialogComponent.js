...
import seederData from "./admissionscorepopulatedtableCsv.seeder.json";
...
const AdmissionscorepopulatedtableCsvSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("admissionscorepopulatedtableCsv").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(AdmissionscorepopulatedtableCsvSeederDialogComponent);
