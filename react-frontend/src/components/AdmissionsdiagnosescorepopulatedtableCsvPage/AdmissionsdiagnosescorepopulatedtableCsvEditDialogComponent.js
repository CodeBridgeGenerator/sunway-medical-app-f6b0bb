...
primarydiagnosiscode: _entity?.primarydiagnosiscode,
primarydiagnosisdescription: _entity?.primarydiagnosisdescription,
        };

        setLoading(true);
        try {
            
        await client.service("admissionsdiagnosescorepopulatedtableCsv").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info admissionsdiagnosescorepopulatedtableCsv updated successfully" });
...
const admissionidOptions = admissionid.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Diagnoses" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="admissionsdiagnosescorepopulatedtableCsv-edit-dialog-component">
