...
import seederData from "./admissionsdiagnosescorepopulatedtableCsv.seeder.json";
...
const AdmissionsdiagnosescorepopulatedtableCsvSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("admissionsdiagnosescorepopulatedtableCsv").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(AdmissionsdiagnosescorepopulatedtableCsvSeederDialogComponent);
