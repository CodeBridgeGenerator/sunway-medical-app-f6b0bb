...
import patientcorepopulatedtableCsvFakerFactory from "./patientcorepopulatedtableCsvFakerFactory";
...
const PatientcorepopulatedtableCsvFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = patientcorepopulatedtableCsvFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("patientcorepopulatedtableCsv").create(elem));
...
export default connect(mapState, mapDispatch)(PatientcorepopulatedtableCsvFakerDialogComponent);
