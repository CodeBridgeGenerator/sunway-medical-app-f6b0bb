...
import seederData from "./patientcorepopulatedtableCsv.seeder.json";
...
const PatientcorepopulatedtableCsvSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("patientcorepopulatedtableCsv").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(PatientcorepopulatedtableCsvSeederDialogComponent);
