...
const PatientcorepopulatedtableCsvCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            patientid: _entity?.patientid,
patientgender: _entity?.patientgender,
patientdateofbirth: _entity?.patientdateofbirth,
patientrace: _entity?.patientrace,
patientmaritalstatus: _entity?.patientmaritalstatus,
patientlanguage: _entity?.patientlanguage,
patientpopulationpercentagebelowpoverty: _entity?.patientpopulationpercentagebelowpoverty,
        };

        setLoading(true);
        try {
            
        const result = await client.service("patientcorepopulatedtableCsv").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info patientcorepopulatedtableCsv updated successfully" });
    ...
    return (
        <Dialog header="Edit Patients" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="patientcorepopulatedtableCsv-edit-dialog-component">
